...
# Define the model
model=Model(inputs=[input_ids, attention_mask,input_vad],outputs=[stance_output,moral_output])

# Model Compile
model.compile(optimizer=Adam(0.0001),loss={'task_stance':'categorical_crossentropy','task_moral':'binary_crossentropy'},
    loss_weights={'task_stance':1.0,'task_moral':0.3}, metrics=['accuracy'])    
print(model.summary())

# Model Fit
model.fit([train_encodings['input_ids'], train_encodings['attention_mask'],li_vad_train],[train_stance,li_moral_train],batch_size=64,epochs=20,verbose=2)
