...
print("sequence_output1:::",sequence_output1)
# VAD input feature
...
attention_output_tox= Dense(128,activation="relu")(attention_output)
tox_output=Dense(2, activation="softmax", name="task_tox")(attention_output_tox)

attention_output_moral= Dense(128,activation="relu")(attention_output)    
moral_output=Dense(10, activation="sigmoid", name="task_moral")(attention_output_moral)  
...
# Define the model
model=Model(inputs=[input_ids, attention_mask,input_vad],outputs=[stance_output,tox_output,moral_output,act_output])

# Model Compile
model.compile(optimizer=Adam(0.0001),loss={'task_stance':'categorical_crossentropy','task_tox':'binary_crossentropy','task_moral':'binary_crossentropy','task_act':'categorical_crossentropy'},
    loss_weights={'task_stance':1.0,'task_tox':0.5,'task_act':0.4,'task_moral':0.3}, metrics=['accuracy'])    
print(model.summary())

# Model Fit
model.fit([train_encodings['input_ids'], train_encodings['attention_mask'],li_vad_train],[train_stance,train_tox,li_moral_train,train_act],batch_size=16,epochs=20,verbose=2)
