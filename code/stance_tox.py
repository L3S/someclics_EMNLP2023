...
# Define the model
model=Model(inputs=[input_ids, attention_mask,input_vad],outputs=[stance_output,tox_output])

# Model Compile
model.compile(optimizer=Adam(0.0001),loss={'task_stance':'categorical_crossentropy','task_tox':'binary_crossentropy'},
    loss_weights={'task_stance':1.0,'task_tox':0.5}, metrics=['accuracy'])    
print(model.summary())

# Model Fit
model.fit([train_encodings['input_ids'], train_encodings['attention_mask'],li_vad_train],[train_stance,train_tox],batch_size=32,epochs=20,verbose=2)
